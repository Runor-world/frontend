  ...
  async (image, thunkAPI) => {
    try {
      ...
      const res = await axios.patch(
        `${baseUrl}/api/profile/backgroundphoto`,
        formData,
        {
          headers: {
...
export const updateProfilePhoto = createAsyncThunk(
  ...
  async (image, thunkAPI) => {
    try {
      const formData = new FormData();
      formData.append("image", image.file);
      const res = await axios.patch(`${baseUrl}/api/profile/photo`, formData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
...
export const createUserServiceProfile = createAsyncThunk(
  ...
  async (values, thunkAPI) => {
    try {
      const res = await axios.post(`${baseUrl}/api/profile/service`, values, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      ...
      .addCase(updateProfile.fulfilled, (state, { payload }) => {
        ...
        if (payload) {
          ...
          state.personalProfile = {
            ...
            birthday:
              toISODate(personalProfile.birthday) ??
              state.personalProfile.birthday,
          };
        } else {
          state.errorMessage = "Something went wrong";
        }
