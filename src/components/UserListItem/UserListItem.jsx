...
const UserListItem = ({_id, firstName, lastName, active, role, profile,  email}) => {
    const dispatch = useDispatch()

    const handleClick = () => {
        dispatch(setSelectedUser({_id, firstName, lastName, active, role, profile}))
    ...
    return (
        <article className='flex gap-4 lg:gap-5 flex-wrap lg:flex-nowrap justify-start w-full lg:w-1/3 items-center group hover:scale-[101%] bg-slate-100 duration-200 transitions-all border-b-2 p-2 rounded-lg'>
            ...
            <div className='flex flex-col gap-0 shrink-0 flex-1'>
                ...
                <small className='text-slate-500'>{role} 
                    <span className={`${active? 'text-green-900': 'text-red-900'} font-medium`}>
                ...
                <div className='mt-4 w-full flex gap-2'>
                    ...
                    <button 
                        onClick={handleClick} disabled={ role==='admin'} 
